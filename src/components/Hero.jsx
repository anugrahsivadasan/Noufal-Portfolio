// components/Hero.js - Responsive improved (no UI change)
...
const mediaItems = [
  { src: figma, x: -433, y: -25, size: "w-16 h-16 sm:w-20 sm:h-20" },
];

const backgroundIcons = [
  { src: canva, x: -550, y: -450, size: "w-10 h-10 sm:w-16 sm:h-16", speed: 1 },
  { src: figma, x: -300, y: -200, size: "w-10 h-10 sm:w-16 sm:h-16", speed: 1 },
  { src: xdIcon, x: 400, y: -350, size: "w-8 h-8 sm:w-12 sm:h-12", speed: 1.5 },
  { src: uxIcon, x: 200, y: 250, size: "w-12 h-12 sm:w-20 sm:h-20", speed: 0.5 },
  { src: adobe, x: -500, y: 150, size: "w-12 h-12 sm:w-20 sm:h-20", speed: 0.3 },
];
...
export default function Hero() {
  ...
  return (
    <section ref={sectionRef} className="relative min-h-screen bg-black overflow-hidden flex items-center justify-center px-4">
      
      {/* Hide heavy background on mobile */}
      <div className="hidden sm:block">
        {backgroundIcons.map((icon, i) => (
          <div key={i} ref={(el) => (bgIconRefs.current[i] = el)}
      ...
      <div ref={heroRef} className="relative text-center">
        ...
        <h1 className="font-extrabold tracking-tight text-[18vw] sm:text-[14vw] md:text-[12vw] leading-none">
          <span style={{ color: accentGradient }}>
            NOUFAL VI
          </span>
        </h1>

        {/* Skills — stack nicely on mobile */}
        <div className="sm:hidden mt-6 flex flex-wrap justify-center gap-2">
          {skills.map((skill, i) => (
            <span key={i} className={`${skill.color} px-3 py-1 rounded-full text-xs`}>
              {skill.label}
            </span>
          ))}
        </div>
        ...
        {mediaItems.map((item, i) => (
          <div key={i}
            className={`hidden sm:block absolute left-1/2 top-1/2 ${item.size} rounded-xl shadow-2xl animate-float-soft`}
        ...
        <div className="hidden sm:block">
          {backgroundText.map((item, i) => (
            <span key={i} ref={(el) => (bgTextRefs.current[i] = el)}
      ...
      <div className="absolute bottom-6 text-[10px] sm:text-xs text-white/70 border border-white/20 px-4 py-2 rounded-full backdrop-blur">
